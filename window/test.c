...
int main(void)
{
        rp_window_init(640u, 480u, "Testing Window");

        while (rp_window_is_open()) {
                rp_window_poll();
                rp_window_swap_buffers();
        }
...
enum { AT_NONE = 0, AT_CAMERA, AT_MODEL, ACTOR_TYPE_COUNT };
...
static const struct face copy_faces[2] = { {       /* face[0] */
                                             {     /* verts */
                                               {   /* verts[0] */
                                                 { /* pos */
                                                   0.f,
                                    ...
                                    const struct face *faces)
{
        ...
        m = (struct model *)rp_mem_alloc(sizeof(*m), "Actor->Model");
        assert(m);

        m->faces_cnt = faces_cnt;
        m->faces     = (struct face *)rp_mem_alloc(sizeof(*faces) * faces_cnt,
                                               "Actor->Model->Faces");
        memcpy(m->faces, faces, sizeof(*faces) * faces_cnt);

        is_equal = model_face_arrays_are_equal(m->faces_cnt,
                                               m->faces,
                                               faces_cnt,
                                               faces);
        assert(is_equal);
...
static struct actor *actor_allocate(struct model *mdl)
{
        ...
        a = (struct actor *)rp_mem_alloc(sizeof(*a), "Actor");
        assert(a);
